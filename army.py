...
class Army:
    def __init__(self, fraction, init_number_of_base_units, init_number_of_special_units, start_point_x, start_point_y):
        ...
        self.__base_unit_list = []
        self.__special_unit_list = []
        self.__start_point_x = start_point_x
        self.__start_point_y = start_point_y

    def generate_base_unit_list(self):
        for id in range(self.__init_number_of_base_units):
            new_base_unit = Base_unit(id, self.__fraction, self.__start_point_x, self.__start_point_y)
            self.__base_unit_list.append(new_base_unit)

    def generate_special_unit_list(self):
        pass
    ...
    def move(self):
        if self.__is_alive == True:
            ...
            while movement > 0:
                ...
                move = battleground.is_it_free(pos_x, pos_y)
                if move == 1:
